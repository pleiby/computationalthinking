...
function convolve(M, kernel)
	...
	m, n = size(M)
    @inbounds for i in 1:m # suspend bounds checking for speed
        for j in 1:n
            # (k, l) loop over the neighbouring pixels
			accumulator = 0 * M[1, 1]
			for k in -half_height:-half_height + height - 1
				for l in -half_width:-half_width + width - 1
					Mi = i - k # note subtracting rather thank adding k flips kernel
					Mj = j - l
					# First index into M
					if Mi < 1 # clamp(Mi, 1, m)
						Mi = 1
					elseif Mi > m
						Mi = m
					end
					# Second index into M
					if Mj < 1 # clamp(Mj, 1, n)
						Mj = 1
					elseif Mj > n
						Mj = n
					end
...
# ╔═╡ 8b8d11e2-5c6d-11eb-388c-cdeb00177dac


# ╔═╡ c116572e-5c6d-11eb-1a62-2505dec7ff19
"""
    foobar(x[, y])
...
return measaure of overall brightness for RGB
"""
